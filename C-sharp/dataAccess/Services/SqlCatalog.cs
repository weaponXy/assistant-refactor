...
namespace dataAccess.Services
{
    ...
    public sealed partial class SqlCatalog : ISqlCatalog
    {
        ...
        private static DateOnly GetDateArg(IDictionary<string, object?> a, string key)
        {
            ...
            throw new InvalidCastException($"Arg '{key}' must be ISO date (yyyy-MM-dd).");
        }
        ...
        // prev window helper (same span as [start..end], immediately before start)
        private static (DateOnly prevStart, DateOnly prevEnd) GetPriorWindow(DateOnly start, DateOnly end)
        {
            var spanDays = (end.ToDateTime(TimeOnly.MinValue) - start.ToDateTime(TimeOnly.MinValue)).TotalDays + 1;
            var prevEnd = start.AddDays(-1);
            var prevStart = DateOnly.FromDateTime(prevEnd.ToDateTime(TimeOnly.MinValue).AddDays(-spanDays + 1));
        ...
        public Task<object?> RunAsync(string queryId, IDictionary<string, object?> args, CancellationToken ct = default) =>
            queryId switch
            {
                ...
                "BUDGET_UTILIZATION" => ExpenseBudgetVsActual(args, ct), // alias
                ...
                "SALES_BY_DAY_PRIOR" => SalesByDayPrior(args, ct),
        ...
        private async Task<object?> ExpenseSummary(IDictionary<string, object?> a, CancellationToken ct)
        {
            ...
            var (prevStart, prevEnd) = GetPriorWindow(start, end);
        ...
        private async Task<object?> ExpenseByCategoryWeekly(IDictionary<string, object?> a, CancellationToken ct)
        {
            ...
            var rows = await _db.Expenses
                ...
                .Join(_db.Categories,
                      e => e.CategoryId!,
                      c => c.Id,
                      (e, c) => new { c.Name, e.OccurredOn, e.Amount })
                .ToListAsync(ct);
        ...
        private async Task<object?> SalesSummary(IDictionary<string, object?> a, CancellationToken ct)
        {
            ...
            var (prevStartD, prevEndD) = GetPriorWindow(startD, endD);
        ...
        private async Task<object?> SalesByDayPrior(IDictionary<string, object?> a, CancellationToken ct)
        {
            // Prefer explicit prev_start/prev_end; else derive from start/end
            DateOnly prevStartD, prevEndD;

            if (a.ContainsKey("prev_start") && a.ContainsKey("prev_end"))
            {
                prevStartD = GetDateArg(a, "prev_start");
                prevEndD = GetDateArg(a, "prev_end");
            }
            else
            {
                var startD = GetDateArg(a, "start");
                var endD = GetDateArg(a, "end");
                (prevStartD, prevEndD) = GetPriorWindow(startD, endD);
            }
